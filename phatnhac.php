...
<?php 
 ...
 $nhan=$_GET['id'];
$list=mysql_query("select * from baihat where baihat.idbh='".$nhan."'");
$ma = mysql_fetch_row($list);
$moi=$ma[3];
$hat=mysql_query("select * from casi where casi.idcs='".$moi."'");
...
<div class="phatnhac">
<div class="tenbaihat"><h2>Đang Phát:<?php echo $ma[1];?>  -  <?php echo $san[2];?> <?php   
$qwe=$ma[5]+1; mysql_query("update baihat set luotnghe='".$qwe."' where idbh='".$ma[0]."'");
echo " - lượt nghe:".++$ma[5];
?></h2></div>
</pre><br><br><center><video width="600" height="1" background=#fff controls autoplay>
  <source src="<?php echo $ma[2];?>" type="video/mp4">
</video></center>
</div>	
<div class="tat">
<div class="ttcasi">
...
<a title="<?php echo $aa['tenbh']; ?>" href="phatnhac.php?id=<?php echo $aa['idbh'] ;?>" ><?php echo $aa['tenbh']; ?></a>
   <h5>trình bày:<a href="casi.php?idcs=<?php echo $aa['idcs']; ?>"><?php $aoa=$aa['idcs'];$raa = mysql_query("select tencs from casi where idcs='".$aoa."'");$ga=mysql_fetch_array($raa); echo $ga['tencs'];?></a> <?php echo "lượt nghe:" .$aa['luotnghe']; ?> </h5>
 </center> </div>
